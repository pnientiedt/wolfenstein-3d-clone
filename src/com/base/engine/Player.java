...
public class Player {
	...
	public static final float SIZEX = (float) ((double) SIZEY / (1.03797 * 2.0));
	...
	public void input() {
		if (Input.getKeyDown(Input.KEY_E)) {
			Game.getLevel().openDoors(getCamera().getPos(), true);
		}
		...
		if (Input.getKey(Input.KEY_W))
			movementVector = movementVector.add(camera.getForward());// camera.move(camera.getForward(),
																		// movAmt);
		if (Input.getKey(Input.KEY_S))
			movementVector = movementVector.sub(camera.getForward());// camera.move(camera.getForward(),
																		// -movAmt);
		if (Input.getKey(Input.KEY_A))
			movementVector = movementVector.add(camera.getLeft());// camera.move(camera.getLeft(),
																	// movAmt);
		if (Input.getKey(Input.KEY_D))
			movementVector = movementVector.add(camera.getRight());// camera.move(camera.getRight(),
																	// movAmt);
	...
	public void update() {
		...
		//Gun movement
		gunTransform.setTranslation(camera.getPos().add(camera.getForward().mul(0.105f)));
